...
SFT 损失由 1.07 降至 0.39，SimPO 偏好损失全程稳定在 ~1.0，
ORPO 损失由 ~1.1 降至 ~0.5（全程零 nan）。

## 3. 显存与效率（框架优势量化）

| 阶段 | 数据/步数 | 吞吐 | 单卡峰值显存 | 收敛情况 |
|---|---|---|---|---|
| LoRA SFT | 10k×2ep / 1250 步 | 4.14 s/it（全程约 86 分钟） | **83.7GB / 96GB** | loss 1.07 → 0.39 |
| DPO/SimPO | 4257 对 / 133 步 | 14.2 s/it（双序列前向，31 分钟） | **87.6GB / 96GB** | loss 稳定在 ~1.0 |
| ORPO | 4257 对 / 133 步 | 21.9 s/it（bs1×acc4，40 分钟） | **84.7GB / 96GB** | loss ~1.1 → ~0.5，全程零 nan |
...
torch.profiler 定位三大瓶颈后实施同步消除 / grouped GEMM / attention 提速（详见 `docs/07_improvements.md` 第 10 节），
loss 与上表完全一致：

| 阶段 | 优化前 | 优化后 |
|---|---|---|
| LoRA SFT | 4.14 s/it（约 86 分钟） | **2.98 s/it（约 62 分钟）**，loss 终值 0.383 |
| DPO/SimPO | 14.2 s/it（31 分钟） | **11.26 s/it（25 分 13 秒）**，loss 终值 0.997 |
| ORPO | 21.9 s/it（40 分钟） | **13.12 s/it（29 分 04 秒）**，loss 末值 0.70 |
| nccl all_to_all（4 步窗口） | 4.59s | **1.34s（-71%）** |
